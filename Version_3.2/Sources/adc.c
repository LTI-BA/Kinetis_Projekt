...
int ADC_calibrate(void) {
	...
	variable += ADC0_CLPS + ADC0_CLP0 + ADC0_CLP1 + ADC0_CLP2 + ADC0_CLP3
			+ ADC0_CLP4;
	...
	variable = 0;
	variable += ADC0_CLMS + ADC0_CLM0 + ADC0_CLM1 + ADC0_CLM2 + ADC0_CLM3
			+ ADC0_CLM4;
	variable /= 2;
	variable |= 0b1000000000000000;
	ADC0_MG = variable;
	...
	variable = 0;
	// Add the plus-side calibration results CLP0, CLP1, CLP2, CLP3, CLP4, and CLPS to the variable.
	variable += ADC1_CLPS + ADC1_CLP0 + ADC1_CLP1 + ADC1_CLP2 + ADC1_CLP3
			+ ADC1_CLP4;
	// Divide the variable by two.
	variable /= 2;
	// Set the MSB of the variable.
	...
	// Repeat the procedure for the minus-side gain calibration value.
	variable = 0;
	variable += ADC1_CLMS + ADC1_CLM0 + ADC1_CLM1 + ADC1_CLM2 + ADC1_CLM3
			+ ADC1_CLM4;
	variable /= 2;
	variable |= 0b1000000000000000;
	ADC1_MG = variable;
...
unsigned short ADC_read(unsigned char channel) { // writes to SC1A subsequently initiate a new conversion, if SC1[ADCH] contains a value other than all 1s. (other than disabled)
